...
from Allgather_new_scaleCCL.utils.NCCL import nccl_topo, fast_nccl_topo
...
def main(collective_time, chunk, propagation_latency: Optional[float] = None):
    ...
    for t in range(num_steps):
        ...
        if t % 10 == 0:
            ...
            policy, arrival_times, time_val = global_policy(NCCL_topology, datacenter, current_stream, per_link_pos)
        # policy = build_path_ass_input(arrival_times, current_stream, topology, policy)    
        # NCCL_topology = fast_nccl_topo(topology, NCCL_topology)
        # policy, arrival_times, time_val = global_NCCL_policy(NCCL_topology, datacenter, next_stream, per_link_pos)
