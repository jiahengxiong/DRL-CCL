...
def simulation(collective_time, policy, agent_set, action_set):
    ...
    while check_buffer(topology=NVD2_topology) is False:
        ...
        if check_buffer(topology=NVD2_topology):
            ...
            collective_time[config.connectivity][config.num_chunk][config.packet_size] = f'{normalized_time} us'
            # for BR in BR_ac:
...
#                 policy= []
#                 simulation(collective_time, policy)
#
#                 # 记录结束时间
#                 end_time = time.time()
#
