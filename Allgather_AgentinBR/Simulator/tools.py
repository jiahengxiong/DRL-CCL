...
def agent_BR(topology, dst, agent_set, action_set, buffer, time,  BR_ac):
    ...
    probs = probs.tolist()
    act_possibility = np.array(probs)
    act_possibility = np.maximum(act_possibility, 0)  # 防止出现负数
    act_possibility = act_possibility / act_possibility.sum()  # 保证和为1
    act = np.random.choice(len(act_possibility), p=act_possibility)
    # act = probs.argmax()
    # _, act = scores.max(dim=0)
    (src_node, dst_node) = action_set[dst][act]
    if buffer not in  topology.nodes[dst]['added_job'][(src_node, dst_node)]:
        ...
        # probs = probs.tolist()
        BR_ac[dst].append([link_feats, global_feats, act, probs, -link_feats[act][-1]])
